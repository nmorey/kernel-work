...
```bash
kernel cve fetch [-u <bugzilla-email>] [-f] [-b <bug id or CVE>]
```
...
* **Selective Refresh (`-b`, `--bz`):** Only refresh specific bugs or CVEs (e.g. `-b 12345` or `-b CVE-2026-99999`, can be specified multiple times) without wiping the entire local tracker cache. Passing `-b` automatically implies `--force` for the specified bugs.
